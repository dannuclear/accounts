...
class Employee(models.Model):
    ...
    empChangesDate = models.DateField(
        blank=False, db_column="emp_changes_date", null=False)
...
class Prepayment(models.Model):
    ...
    pdSource = models.SmallIntegerField(
        db_column="pd_source", blank=False, null=False)
    # emp_org_no	CHAR(8)	integer	да	Табельный номер	Всегда 8 цифр. Незначащие символы должны быть заполнены [0] слева.
    empOrgNo = models.IntegerField(
        db_column="emp_org_no", blank=False, null=False)
    # xv26ei_id	VARCHAR(4)	SMALLINT	нет	Суррогатный идентификатор пункта сметы	Только цифры
    xv26eiId = models.SmallIntegerField(
        db_column="xv26ei_id", blank=True, null=True)
    ...
    orderId = models.CharField(
        max_length=20, blank=True, db_column="order_id", null=True)
    ...
    orderIdUpd = models.CharField(
        max_length=20, blank=True, db_column="order_id_upd", null=True)
    # order_no	VARCHAR(100)	varchar(100)	нет	Номер документа (WC07P)
    orderNo = models.CharField(
        max_length=100, blank=True, db_column="order_no", null=True)
    ...
    sum = models.DecimalField(
        max_digits=10, decimal_places=2, db_column="sum", blank=False, null=False)
    # acpl_account	CHAR(2)	smallint	нет	Балансовый счет	незначащие символы должны быть заполнены [0] слева
    acplAccount = models.SmallIntegerField(
        db_column="acpl_account", blank=True, null=True)
    # acpl_subaccount	CHAR(2)	smallint	нет	Балансовый субсчет	незначащие символы должны быть заполнены [0] слева
    acplSubaccount = models.SmallIntegerField(
        db_column="acpl_subaccount", blank=True, null=True)
...
class Estimate(models.Model):
    ...
    xv26eiId = models.IntegerField(
        primary_key=True, db_column="xv26ei_id", null=False)
    # xv26eic_year	CHAR(4)	SMALLINT	да	Отчетный год	Всегда 4 цифры
    xv26eicYear = models.SmallIntegerField(
        db_column="xv26eic_year", blank=False, null=False)
    # xv26eih_date_begin	CHAR(10)	DATE	да	Дата начала периода действия сметы	использовать формат (YYYY-MM-DD)
    xv26eihDateBegin = models.DateField(
        blank=False, db_column="xv26eih_date_begin", null=False)
    # xv26eih_date_end	CHAR(10)	DATE	нет	Дата конца периода действия сметы	использовать формат (YYYY-MM-DD)
    xv26eihDateEnd = models.DateField(
        blank=True, db_column="xv26eih_date_end", null=True)
    # xv26eih_name	VARCHAR(200)	VARCHAR(200)	да	Наименование пункта сметы
    xv26eihName = models.CharField(
        max_length=200, blank=False, db_column="xv26eih_name", null=False)
    ...
    xv26eirSumPlan = models.DecimalField(
        max_digits=19, decimal_places=4, db_column="xv26eir_sum_plan", blank=False, null=False)
...
class WC07POrder(models.Model):
    ...
    orderName = models.CharField(
        db_column="order_name", max_length=200, blank=True, null=True)
    # документ_ID
    orderId = models.CharField(
        db_column="order_id", primary_key=True, max_length=20)
    # номер_документа
    orderNum = models.CharField(
        db_column="order_num", max_length=50, blank=True, null=True)
    # дата_документа
    ...
    depName = models.CharField(
        db_column="dep_name", max_length=200, blank=True, null=True)
    # фио
    fio = models.CharField(
        db_column="fio", max_length=200, blank=True, null=True)
    # должность
    profName = models.CharField(
        db_column="prof_name", max_length=256, blank=True, null=True)
    # место_назначение
    distName = models.CharField(
        db_column="dist_name", max_length=500, blank=True, null=True)
    # дата_начала_командирования
    missionBegin = models.DateField(
        db_column="mission_begin", blank=True, null=True)
    # дата_окончания_командирования
    missionEnd = models.DateField(
        db_column="mission_end", blank=True, null=True)
    # цель_командирования
    missionPurpose = models.CharField(
        db_column="mission_purpose", max_length=500, blank=True, null=True)
    # смета_ID
    estimateId = models.IntegerField(
...
class Protocol (models.Model):
    ...
    operDate = models.DateTimeField(
        db_column="oper_date", blank=False, null=False)
    ...
    class Meta:
        ...
        default_permissions = ()
        permissions = [
            ("view_exchange_protocol", "Просмотр")
        ]

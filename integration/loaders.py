...
wc07pOrderFields = ['orderName', 'orderId', 'orderNum', 'orderDate', 'empOrgNo', 'depName', 'fio', 'profName', 'distName', 'missionBegin', 'missionEnd', 'missionPurpose', 'estimateId', 'payDoc', 'orderIdUpd']
...
def load (path, type):
    with open(path, encoding="utf-8" ,mode ='r') as file:
        ...
        for row in reader:
            ...
            obj = Estimate(**replacedRow) if type == FileType.ESTIMATE else Employee(**replacedRow) if type == FileType.EMPLOYEE else Prepayment(**replacedRow) if type == FileType.PREPAYMENT else WC07POrder(**replacedRow) if type == FileType.WC07P_ORDER else None
            obj.save()
